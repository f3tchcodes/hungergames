...
import { _EphToast, _EphToastDefer } from "./common.js";
...
export async function userPermissions(interaction: Interaction, permissions: PermissionResolvable[], permission_names: string[], reply: "replied" | "deferred" = "replied") {
    ...
    if (reply === "replied") await _EphToast(interaction, message);
    if (reply === "deferred") await _EphToastDefer(interaction, message);
    return false;
}
