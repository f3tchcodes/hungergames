...
export async function buildTributeList(canvas: Canvas, districts: PlayersDistricts[][], rows: number, alive_status: boolean) {
    canvas.setTextAlign("center");

    const total_districts = districts.length;
    const tributes: PlayersDistricts[] = [];
    districts.forEach(district => district.forEach(player => tributes.push(player)));
    const district_size = Math.floor(tributes.length / total_districts);
    ...
    for (let i = 0; i < districts.length; i++) {
        ...
        if (!district) return console.error(`district doesn't exist ${i}`);
        ...
        for (let j = 0; j < district.length; j++) {
            ...
            if (alive_status) {
                ...
                if (!alive) { text = "Deceased"; text_color = config.CANVAS_DEAD_COLOR; }
